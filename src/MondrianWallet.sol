...
contract MondrianWallet is Ownable, ERC721, IAccount {
    ...
    function execute(address dest, uint256 value, bytes calldata func) external requireFromEntryPointOrOwner {
        // @audit ZkSync does not have native support for direct Ether transfers, extra considerations are needed
        // https://docs.zksync.io/build/developer-reference/differences-with-ethereum.html#call-staticcall-delegatecall
        (bool success, bytes memory result) = dest.call{value: value}(func);
        if (!success) {
            assembly {
                // @audit will this work on ZkSync? It appears it should
        ...
        returns (uint256 validationData)
    {
        ...
        // @audit ECDSA.recover recovers the address from the signature,
        // but there is no check if the recovered address matches the expected signer's address.
        // This could allow unauthorized transactions if the message is signed by any address.
        // ERC-4337 as a signature validation should leverage the contract's capabilities to enforce rules and check state,
        // not just perform mathematical signature verification.
        ECDSA.recover(hash, userOp.signature);
        return SIG_VALIDATION_SUCCESS; // e 0 if succes, 1 if fail. But this always returns success @audit
    }
     ...
     */
    // This function allows the user to deposit Ether into the EntryPoint contract.
    // The EntryPoint contract will use the deposited Ether to cover the gas costs
    // and execution fees of user operations submitted by this account
    // (operations where this account is specified in the PackedUserOperation.sender field. )
